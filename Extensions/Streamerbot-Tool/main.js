let headerHtml = `<header><div class="main"><img src="https://ik1497.github.io/assets/images/favicon.png" alt="favicon"><div class="name-description"><p class="name">Streamer.bot Tool</p><p class="description">by Ik1497</p></div></div><aside><div class="form-area"><label>Url</label><input type="url" value="localhost" class="url"></div><div class="form-area"><label>Port</label><input type="number" value="8080" max="9999" class="port"></div><div class="form-area"><label>Endpoint</label><input type="text" value="/" class="endpoint"></div><button class="connect-websocket">Connect</button></aside></header>`
...
function connectws() {
  if ("WebSocket" in window) {
    ...
    ws.addEventListener("message", (event) => {
      ...
      // Global Arguments //
      //////////////////////
      let globalArgsHtml = `<div class="global-args"><div class="header"><div class="info"><p class="title">Global Arguments</p><p class="description">Arguments that will be assigned to all actions</p></div><button class="close-button">&times</button></div><div class="main"><table><thead><tr><th>Argument</th><th>Value</th></tr></thead><tbody><tr><td><input type="text" placeholder="Argument"></td><td><input type="text" placeholder="Value"></td><td><button title="Remove Argument"class="remove-row mdi mdi-trash-can"></button></td></tr><tr><td></td><td><button title="Add Argument" class="add-row mdi mdi-plus-box"></button></td></tr></tbody></table></div><div class="save-button"><button class="mdi mdi-content-save" title="Save arguments">Save</button></div></div>`
      document.querySelector(`.open-global-args`).addEventListener(`click`, function () {
        ...
        document.querySelector(`.global-args .add-row`).addEventListener(`click`, function () {
          document.querySelector(`.global-args table tbody tr:has(td button.add-row)`).insertAdjacentHTML(`beforebegin`, `<tr><td><input type="text" placeholder="Argument"></td><td><input type="text" placeholder="Value"></td><td><button title="Remove Argument"class="remove-row mdi mdi-trash-can"></button></td></tr>`)
          globalArgumentsRemoveRow()
        })

        globalArgumentsRemoveRow()

        function globalArgumentsRemoveRow() {
          document.querySelectorAll(`.global-args table tbody tr:not(:last-child)`).forEach(row => {        
            row.querySelector(`.remove-row`).removeEventListener(`click`, globalArgumentsRemoveRowFunction())
            row.querySelector(`.remove-row`).addEventListener(`click`, globalArgumentsRemoveRowFunction())
            function globalArgumentsRemoveRowFunction() {
