...
function(err, data) {
    ...
    for (var eventsLength = 0; eventsLength < data.events.length; eventsLength++) {
        ...
        document.querySelector(`.obs-events-nav`).insertAdjacentHTML(`beforeend`, `<li class="obs-event-list-item category-` + events.category + ` item-` + events.eventType + `"><a href="#` + events.eventType + `" onclick="navClick()"><p class="obs-event-list-item-title">` + events.eventType + `</p><p class="obs-event-list-item-description">` + events.description + `</p></a></li>`);
