...
document.querySelector(`header.header-primary aside .settings`).addEventListener(`click`, function () {
    ...
    if (localStorage.getItem(`websiteSettings__scrollbar`) || `false` === `true`) {
        checked = ` checked`
    }
    ...
    let modalContents = `${setting__dropdown}${setting__scrollbar}`
...
function settings__reloadScrollbar() {
