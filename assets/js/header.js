...
document.querySelector(`header.header-primary aside .settings`).addEventListener(`click`, function () {
    let modalContents = `<p>The settings below won't work currently, they're coming soon</p><p><input type="checkbox" id="cb-show-scrollbar"> <label for="cb-show-scrollbar">Show Scrollbar</label><br><input type="checkbox" id="cb-lightmode"> <label for="cb-lightmode">Enable lightmode</label></p>`
